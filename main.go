...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	speed := r.meanSpeed()
	return ((CaloriesMeanSpeedMultiplier*speed + CaloriesMeanSpeedShift) * float64(r.Weight) / MInKm * r.Duration.Hours() * MinInHours)
}
...
func (r Running) TrainingInfo() InfoMessage {
	...
	distance := r.distance()
	speed := r.meanSpeed()
	durationMinutes := float64(r.Duration.Minutes())
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	speedInMetersPerSec := w.meanSpeed() / KmHInMsec
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speedInMetersPerSec, 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
...
func (w Walking) TrainingInfo() InfoMessage {
	...
	// Add a check for Duration
	durationMinutes := float64(w.Duration.Minutes())
	if durationMinutes > 0 {
		return InfoMessage{
			w.TrainingType,
			w.Duration,
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	speed := s.meanSpeed()
	return ((speed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * float64(s.Weight) * s.Duration.Hours())
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	distance := s.meanSpeed() * s.Duration.Hours()
	durationMinutes := float64(s.Duration.Minutes())
	if durationMinutes > 0 {
		return InfoMessage{
			s.TrainingType,
			s.Duration,
...
func main() {
	...
	walking := Walking{
		Training: Training{
			...
			Action:       20000,
			LenStep:      LenStep,
			Duration:     0*time.Hour + 0*time.Minute,
			Weight:       85,
		},
		Height: 185,
	}
